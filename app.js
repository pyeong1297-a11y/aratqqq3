...
const RESULTS_PATH = path.join(ROOT_DIR, "results", "isa-kodex-latest.json");
...
async function runBacktest() {
  if (!refreshPromise) {
    refreshPromise = execFileAsync(
      process.execPath,
      ["src/cli.js", "run", "isa-kodex", "--save", "results/isa-kodex-latest.json"],
      {
        cwd: ROOT_DIR,
...
<html lang="ko">
  <head>
    ...
    <title>ISA KODEX Strategy Board</title>
    <style>
      :root {
        --bg: #f4efe6;
        --bg-accent: #e0ecf7;
  ...
  <body>
    <main class="shell">
      <section class="hero">
        <div class="hero-grid">
          <div>
            ...
            <h1>TQQQ 200/220 기반 최종본을 브라우저에서 바로 확인</h1>
            <p class="lede">
              현재 저장된 <code>results/isa-kodex-latest.json</code>를 읽어 ISA 전략, 장기보유 벤치마크,
              세후 기준 결과를 한 화면에서 비교합니다. 페이지 안에서 백테스트를 다시 돌려 최신 결과로 갱신할 수도 있습니다.
            </p>
            <div class="toolbar">
      ...
      <section class="section panel">
        ...
        <div class="rule-grid">
          ...
          <article class="card">
            <h3>재가입</h3>
            <p>시작 후 2년 뒤 해지 가능. TQQQ가 SMA200 위면 해지 연기, 아래로 내려오면 다음 한국 거래일에 해지 후 즉시 새 ISA를 엽니다.</p>
          </article>
          <article class="card">
            <h3>추가금 배치</h3>
            <p>위험 구간 추가금은 TIGER 미국S&amp;P500으로 보냅니다. 해지 자금은 USD 슬리브로 옮기고 BULZ 비진입 시 SGOV에 대기합니다.</p>
          </article>
          <article class="card">
            <h3>비교 대상</h3>
            <p>Early, Strict, Long-only와 함께 단일 ISA KODEX 적립, 같은 현금흐름의 QLD 적립 벤치마크를 같이 보여줍니다.</p>
          </article>
        </div>
      </section>
    ...
    <script>
      ...
      function buildHighlights(scenarios) {
        ...
        const strict = findOne((item) => item.meta?.signalMode?.id === "dual-strict" && item.meta?.mode === "open" && Number(item.meta?.slipRate || 0) === 0 && item.meta?.taxMode === "taxed");
        const early = findOne((item) => item.meta?.signalMode?.id === "dual-early" && item.meta?.mode === "open" && Number(item.meta?.slipRate || 0) === 0 && item.meta?.taxMode === "taxed");
        ...
        for (const item of [strict, early, longOnly, kodexBench, qldBench]) {
